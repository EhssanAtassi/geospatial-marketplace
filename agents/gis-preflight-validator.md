...
| `convert-inline` | GDAL, file header (format/CRS/layers/features), DWG source-CRS prompt, layer-filter prompt, target-DB confirmation. No DB connectivity check (skill doesn't open a connection). |
...
For `inspect` and `convert-inline` modes, read the file's header via fiona / ezdxf without loading full feature data:
